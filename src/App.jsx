...
const App = () => {
  const [people, setPeople] = useState([
    { id: Date.now(), name: '', items: [{ id: Date.now() + 1, name: '', qty: 1, price: '' }] }
  ]);
  ...
  const addPerson = () => {
    ...
    setPeople([...people, { id: newId, name: '', items: [{ id: Date.now() + 1, name: '', qty: 1, price: '' }] }]);
  ...
  const addSubItem = (pId) => setPeople(people.map(p => p.id === pId ? { ...p, items: [...p.items, { id: Date.now(), name: '', qty: 1, price: '' }] } : p));
